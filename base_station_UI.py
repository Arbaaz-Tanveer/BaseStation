...
from communication import WiFiHandler
...
class Robot:
    def __init__(self, robot_id, name="Robot", color="blue", ip_address=None, port=None):
        ...
        self.wifi_handler = WiFiHandler(ip_address, port, self.handle_received_data) if ip_address and port else None

    def handle_received_data(self, data):
        """Callback to process received data."""
        print(f"Received data from {self.name}: {data}")
        # TODO: Parse data and update self.ball_position, position, orientation, obstacles
        # Example: 
        # data_dict = json.loads(data)
        # self.position = data_dict.get('position', self.position)
        # self.orientation = data_dict.get('orientation', self.orientation)

    def connect(self):
        """Connect to the robot using WiFiHandler."""
        if self.wifi_handler and not self.connected:
            if self.wifi_handler.connect():
                self.connected = True
                return True
        return False
    ...
    def send_to_robot(self, msg):
        ...
        if self.wifi_handler:
            ...
            print(f"No WiFi handler for {self.name}")
...
class BaseStationUI:
    def __init__(self, root):
        ...
        self.robots[0] = Robot(1, "Player", "blue", "127.0.0.1", 5000)
    ...
    def setup_ui(self):
        ...
        try:
            team_logo_img = ImageTk.PhotoImage(Image.open("robocup_logo.png").resize((220, 70)))
            team_logo_label = tk.Label(banner_frame, image=team_logo_img, bg="#a8328d")
            team_logo_label.image = team_logo_img
        except Exception:
            team_logo_label = tk.Label(banner_frame, text="Team Logo", fg="white", bg="#a8328d", font=("Arial", 16))
        ...
        try:
            msl_logo_img = ImageTk.PhotoImage(Image.open("era_logo.png").resize((100, 100)))
            msl_logo_label = tk.Label(center_logo_frame, image=msl_logo_img, bg="#a8328d")
            msl_logo_label.image = msl_logo_img
            msl_logo_label.pack()
        except Exception:
            msl_logo_label = tk.Label(center_logo_frame, text="RoboCup MSL Logo", fg="white", bg="#a8328d", font=("Arial", 16))
            msl_logo_label.pack()
        ...
        try:
            institute_logo_img = ImageTk.PhotoImage(Image.open("iitk_logo.png").resize((75, 75)))
            ...
            institute_logo_label = tk.Label(right_frame, text="Institute Logo", fg="white", bg="#a8328d", font=("Arial", 16))
            institute_logo_label.pack(side=tk.RIGHT, padx=5)
        ...
        self.refbox_connect_btn = tk.Button(right_frame, text="Connect to RefBox", command=self.handle_refbox_connect)
        ...
        for i, robot in enumerate(self.robots):
            ...
            try:
                ...
                robot_image_label.image = bot_photo
        ...
        tk.Button(additional_btn_frame, text="Play/Pause", width=12, command=self.play_pause).pack(side=tk.LEFT, padx=5)
        tk.Button(additional_btn_frame, text="Reset Position", width=12, command=self.reset_position).pack(side=tk.LEFT, padx=5)
        tk.Button(additional_btn_frame, text="Camera Check", width=12, command=self.camera_check).pack(side=tk.LEFT, padx=5)

    def handle_refbox_connect(self):
        if self.logic and not self.logic.refbox_handler.connected:
    ...
    def draw_soccer_lines(self, canvas, w, h):
        ...
        canvas.create_oval(center_x - circle_radius, center_y - circle_radius, center_x + circle_radius, center_y + circle_radius, outline="white", width=2)
    ...
    def draw_ball_on_field(self, canvas, w, h):
        ...
        cy = 10 + by * scale_y
        ball_radius = 6
        canvas.create_oval(cx - ball_radius, cy - ball_radius, cx + ball_radius, cy + ball_radius, fill="white", outline="black", width=2)
    ...
    def show_robot_detail(self, robot):
        ...
        tk.Label(info_frame, text=f"Status: {'Connected' if robot.connected else 'Disconnected'}", fg="green" if robot.connected else "red", font=("Arial", 12, "bold")).pack(side=tk.LEFT, padx=20)
        ...
        def draw_local_map_lines(event=None):
            ...
            rr = 15
            local_map_canvas.create_oval(cx - rr, cy - rr, cx + rr, cy + rr, fill=robot.color, outline="white", width=2)
            angle_rad = math.radians(robot.orientation)
            line_len = 30
            x_end = cx + line_len * math.cos(angle_rad)
            y_end = cy + line_len * math.sin(angle_rad)
            local_map_canvas.create_line(cx, cy, x_end, y_end, fill="white", width=3)
            local_map_canvas.create_oval(cx + 50 - rr, cy - rr, cx + 50 + rr, cy + rr, fill="white", outline="black", width=2)
    ...
    def open_parameters_window(self):
        ...
        def load_parameters():
            filename = filedialog.askopenfilename(title="Load Parameters", filetypes=[("Text files", "*.txt"), ("All files", "*.*")])
            if filename:
                try:
        ...
        def send_to_all():
            try:
                new_params = {param: float(entry.get()) for param, entry in entries.items()}
    ...
    def save_parameters_to_file(self):
        filename = filedialog.asksaveasfilename(title="Save Parameters", defaultextension=".txt", filetypes=[("Text files", "*.txt"), ("All files", "*.*")])
        if filename:
    ...
    def save_log(self):
        filename = filedialog.asksaveasfilename(title="Save Log", defaultextension=".log", filetypes=[("Log files", "*.log"), ("All files", "*.*")])
        if filename:
            with open(filename, 'w') as f:
